...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, attn_mask: Optional[Tensor] = None, context: Optional[Tensor] = None, is_causal: bool = False) -> Tensor:
...
class TextEncoderBlock(nn.Module):
    ...
    def forward(self, x: Tensor, attn_mask: Tensor, context: Optional[Tensor] = None) -> Tensor:
        x = self.norm_attn(x + self.dropout(self.attention(x, attn_mask)))

        if self.cross_attention and context is not None:
            x = self.norm_crossattn(x + self.dropout(self.crossattn(x, context=context)))
...
class VisualEncoder(nn.Module):
    ...
    def __post_init__(self):
        ...
        self.pos_embed = nn.Parameter(torch.randn(1, seq_len, self.dim) * .02)
...
class VLM(nn.Module):
    ...
    def __init__(self, config: Dict, tokenizer_path):
        """
    ...
    def preprocess_text(self, texts: Union[str, List[str]]) -> Dict[str, Tensor]:
        ...
        attention_mask = torch.zeros(len(texts), self.text_encoder.max_position_embeddings, dtype=torch.int32)
        encoded = self._tokenizer.encode_batch(texts)

        for i, seq in enumerate(encoded):
            seq_len = min(len(seq), self.text_encoder.max_position_embeddings)
            input_ids[i, :seq_len] = torch.LongTensor(seq.ids[:self.text_encoder.max_position_embeddings])
