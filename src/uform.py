...
class VLM(nn.Module):
    ...
    def encode_multimodal(
        ...
        assert image is not None or image_features is not None, "Either 'image' or 'image_features' should be present"
        assert text is not None or text_features is not None, "Either 'text_data' or 'text_features' should be present"
    ...
    def get_matching_scores(self, x: torch.Tensor):
...
def get_model(model_name):
    config = model2config[model_name]
    model = VLM(config)

    path = hf_hub_download(repo_id='unum-cloud/uform',
                           filename=model2config[model_name]['checkpoint_name'])
